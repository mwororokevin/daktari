<?php
/**
 * daktari functions and definitions
 *
...
if ( ! function_exists( 'daktari_setup' ) ) :
	/**
	 ...
	 */
	function daktari_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on daktari, use a find and replace
		 * to change 'daktari' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'daktari', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'primary'    => esc_html__( 'Primary', 'daktari' ),
			'secondary'  => esc_html__( 'Secondary', 'daktari' )
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'daktari_custom_background_args', array(
...
add_action( 'after_setup_theme', 'daktari_setup' );

function daktari_add_editor_style(){
	add_editor_style('dist/css/editor-style.css');
}
add_action('admin_init', 'daktari_add_editor_style');
 ...
 */
function daktari_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'daktari_content_width', 1140 );
}
add_action( 'after_setup_theme', 'daktari_content_width', 0 );


/**
 * Enqueue scripts and styles.
 */
function daktari_scripts() {
	wp_enqueue_style( 'daktari-style', get_stylesheet_uri() );

	wp_enqueue_style( 'daktari-bootstrap', get_template_directory_uri() . '/dist/css/bootstrap.min.css', false, '1.1', 'all');

	wp_enqueue_style('daktari-fontawesome', get_template_directory_uri() . '/fonts/font-awesome/css/font-awesome.min.css');

	wp_enqueue_style('daktari-better-nav', get_template_directory_uri() . '/dist/css/bootstrap-better-nav.min.css');

	wp_register_script('popper', 'https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js', false, '', true);
	wp_enqueue_script('popper');

	wp_enqueue_script( 'daktari-tether', get_template_directory_uri() . '/src/js/tether.min.js', array(), '20170115', true );

	wp_enqueue_script( 'daktari-bootstrap', get_template_directory_uri() . '/src/js/bootstrap.min.js', array(jquery), '20170915', true );

	// wp_enqueue_script( 'daktari-nav-scroll', get_template_directory_uri() . '/src/js/nav-scroll.js', array(jquery), '20170115', true );
	
	wp_enqueue_script( 'daktari-skip-link-focus-fix', get_template_directory_uri() . '/src/js/skip-link-focus-fix.js', array(), '20151215', true );

	wp_enqueue_script( 'daktari-bootstrap', get_template_directory_uri() . '/src/js/bootstrap-better-nav.min.js', array(jquery), '20170915', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'daktari_scripts' );
